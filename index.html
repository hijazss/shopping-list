...
<body>
  ...
  <main class="container">
    ...
    <section id="viewCrypto" style="display:none;">
      ...
      <div class="grid3">
        <div class="kpi" id="cryptoKpiSentiment"></div>
        <div class="kpi" id="cryptoKpiScope"></div>
        <div class="kpi" id="cryptoKpiNotes"></div>
      </div>
      ...
      <div class="miniLinks" style="margin-top:10px;">
        <button class="pillBtn" id="cryptoRefreshBtn">Refresh crypto</button>
      </div>
    </section>
  ...
  <script>
    ...
    const CRYPTO_URL = () =>
      `${BACKEND_BASE}/crypto/news/briefing?coins=${encodeURIComponent("BTC,ETH,LINK,SHIB")}&include_top_n=12`;
    ...
    function renderMarketRead(payload){
      ...
      el("readKpiFG").innerHTML = `
        ...
        <div class="kpiSub">Best effort fetch.</div>
      `;
    ...
    function renderHoldingsCommon(payload){
      ...
      if (!list.length){
        el("holdingsKpiSimple").innerHTML = `
          ...
          <div class="kpiSub">
            If you see this, confirm backend has <span class="mono">/report/holdings/common</span>.
          </div>
        `;
        el("holdingsList").innerHTML = "<div class='subtle'>No holdings data returned.</div>";
        el("holdingsHint").textContent = "If empty, try 365D or check backend route.";
        return;
      }
    ...
    async function loadHoldingsCommon(windowDays, force){
      ...
      try{
        ...
        el("holdingsHint").textContent = "Could not load holdings. Confirm backend route + CORS.";
      }
    }
    ...
    function renderDaily(payload){
      ...
      if (!days.length){
        ...
        el("dailyHint").textContent = "If empty, try 180D or 365D.";
        return;
      }
    ...
    async function loadDaily(windowDays, force){
      ...
      try{
        ...
        renderDaily({ days: [] });
        el("dailyHint").textContent = "Could not load daily feed. Confirm backend route + CORS.";
      }
    }

    function cryptoCoinHTML(c){
      ...
      const meta = `${headlines.length} headlines · ${sent.score}/100`;
        ...
        <details class="briefingDetails">
          <summary>
            <div class="summaryLeft">
              ...
              <div class="summaryMeta">${meta}${summary ? " · " + summary : ""}</div>
            </div>
            <div class="tags">
              <span class="kpiBadge badgeNeu">${headlines.length}</span>
              <span class="kpiBadge ${sent.cls}">${sent.label} · ${sent.score}</span>
          ...
          <div class="divider"></div>
          <div class="kpiSub"><strong>Today’s read:</strong> ${summary || "No summary available."}</div>
          <div class="divider"></div>
          <div class="kpiSub"><strong>Headlines:</strong></div>
          <div class="headlineList">
            ${headlines.length ? headlines.map(headlineHTML).join("") : "<div class='subtle'>No headlines for this coin.</div>"}
          </div>
        </details>
      `;
    }
    ...
    function renderCrypto(data){
      ...
      const date = String(data?.date || "");
      const note = String(data?.note || "");
      const errs = normalizeNewsErrors(data?.errors);

      const total = coins.reduce((a, c) => a + (Array.isArray(c?.headlines) ? c.headlines.length : 0), 0);

      el("cryptoKpiSentiment").innerHTML = `
        <div class="kpiTop">
          <div>
            <div class="kpiTitle">Overall sentiment</div>
            <div class="kpiValue">${overall.score}/100</div>
      ...
      el("cryptoKpiScope").innerHTML = `
        <div class="kpiTop">
          <div>
            <div class="kpiTitle">Scope</div>
            <div class="kpiValue">${total} headlines</div>
          </div>
          <span class="kpiBadge badgeNeu">Coins</span>
        </div>
        <div class="kpiSub">${coins.length} symbols</div>
      `;

      el("cryptoKpiNotes").innerHTML = `
        <div class="kpiTop">
          <div>
            <div class="kpiTitle">Notes</div>
            <div class="kpiValue">RSS</div>
        ...
        <div class="kpiSub">${note || "Crypto briefing."}</div>
      `;

      el("cryptoCoins").innerHTML = coins.length
        ? coins.map(cryptoCoinHTML).join("")
        : "<div class='subtle'>No coins returned.</div>";

      el("cryptoHint").textContent = errs.length
        ? ("Some feeds failed: " + errs.slice(0, 3).join(" | "))
        : "Crypto briefing loaded.";
    }

    async function loadCrypto(force){
      if (CACHE_CRYPTO && !force){
        renderCrypto(CACHE_CRYPTO);
        return;
      }
      ...
      try{
        ...
        el("cryptoCoins").innerHTML = "<div class='subtle'>Could not load crypto briefing.</div>";
        el("cryptoHint").textContent = "Backend must expose /crypto/news/briefing and allow CORS.";
        el("cryptoKpiSentiment").innerHTML = "";
        el("cryptoKpiScope").innerHTML = "";
        el("cryptoKpiNotes").innerHTML = "";
      }
    ...
    el("cryptoRefreshBtn").addEventListener("click", () => {
      loadCrypto(true);
    });
